...
# Copy package.json files needed by Interview Service
...
# Compile Interview Service
RUN pnpm --filter interview-service build
...
# Copy compiled Interview Service
COPY --from=build /app/apps/interview-service/dist ./apps/interview-service/dist

# Copy compiled shared RabbitMQ package
COPY --from=build /app/packages/shared-rabbitmq/dist ./packages/shared-rabbitmq/dist

# Run from Interview Service directory
WORKDIR /app/apps/interview-service
